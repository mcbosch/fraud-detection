# Fraud Detection Project

This repository is an end-to-end project focused on building and evaluating fraud detection models using the Amazon Science Fraud Dataset Benchmark.

The aim is to start with basic machine learning models and progressively move towards more advanced techniques such as Topological Data Analysis (TDA) and Graph Neural Networks (GNNs).

## Repository Structure

- `data/`: raw and processed datasets
- `notebooks/`: exploratory data analysis and model development notebooks
- `src/`: source code for data processing, feature engineering, and models
- `models/`: serialized trained models
- `tests/`: unit tests and evaluation scripts
- `README.md`: project overview
- `TODO.md`: task list and next steps

## Goals

1. Download and preprocess the Amazon fraud dataset benchmark.
2. Build baseline models (logistic regression, decision trees, etc.).
3. Develop advanced models incorporating Topological Data Analysis.
4. Explore Graph Neural Networks to capture relational structures in data.
5. Provide documentation, reproducible experiments, and evaluation metrics.

## Getting Started

Instructions for setting up the environment, downloading data, and running experiments will be developed as the project progresses.

---
